...
class LinesByType(FileVisitor):
    ...
    def __init__(self, trace=1):
        super(LinesByType, self).__init__(trace=trace)
    ...
    def visitFile(self, filepath):
        super(LinesByType, self).visitFile(filepath)
...
if __name__ == '__main__':
    ...
    print('Visited %d files and %d dirs' % (walker.fcount,
                                            walker.dcount))
    print('-' * 80)
    print('Source files=>%d, lines=>%d' % (walker.srcFiles,
                                           walker.srcLines))
    print('By types:')
