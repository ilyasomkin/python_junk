...
class DigitalDisplay(Frame):
    def __init__(self, master, cfg):
        super(DigitalDisplay, self).__init__(master)
...
class AnalogDisplay(Canvas):
    def __init__(self, master, cfg):
        super(AnalogDisplay, self).__init__(master,
...
class Clock(Frame):
    def __init__(self, config=ClockConfig, master=None):
        super(Clock, self).__init__(master)
        self.cfg = config
    ...
    def makeWidgets(self, master):
        ...
        master.bind('<ButtonPress-1>', self.onSwitchMode)
        master.bind('<ButtonPress-3>', self.onToggleLabel)
        master.bind('<Configure>', self.onResize)
        master.bind('<KeyPress-s>', self.onCountdownSec)
        master.bind('<KeyPress-m>', self.onCountdownMin)
...
class ClockPopup(PopupWindow):
    def __init__(self, config=ClockConfig, name=''):
        super(ClockPopup, self).__init__(appname, name)
...
class ClockMain(MainWindow):
    def __init__(self, config=ClockConfig, name=''):
        super(ClockMain, self).__init__(appname, name)
        clock = Clock(config, self)
        clock.pack(expand=YES, fill=BOTH)


class ClockWindow(Clock):
    def __init__(self, config=ClockConfig, master=None, name=''):
        super(ClockWindow, self).__init__(config, master)
